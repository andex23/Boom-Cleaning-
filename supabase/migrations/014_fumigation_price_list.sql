-- 014: the published fumigation price list, and a fix for tier-priced services.
--
-- Two things:
--
-- 1. Fumigation moves from "quoted by our team" to an instant quote, using the published
--    bedroom table (50,000 for one bedroom up to 170,000 for eight) plus boys' quarters
--    at 30,000 for one room and 50,000 for two. That BQ step is not a unit price — a
--    second room adds 20,000, not another 30,000 — so it needs a quantity-keyed table.
--
-- 2. A live defect: a tier-priced service refused to quote as soon as the request named a
--    room with no explicit price. A 3-bedroom deep clean quoted 161,250, but adding a
--    living room returned "Living room is not covered by the standard Deep cleaning
--    price." and fell to manual review — so virtually every real enquiry for the flagship
--    service stopped short of a price. A bedroom tier is a whole-home price; rooms without
--    their own line are included in it, not unpriced.

create table if not exists public.service_space_tiers (
  ...
  unique (service_id, space_type_id, quantity)
);

create index if not exists service_space_tiers_service_id_idx on public.service_space_tiers (service_id);

alter table public.service_space_tiers enable row level security;
drop policy if exists "staff read space tiers" on public.service_space_tiers;
create policy "staff read space tiers" on public.service_space_tiers for select to authenticated using (public.is_staff());
drop policy if exists "admins manage space tiers" on public.service_space_tiers;
create policy "admins manage space tiers" on public.service_space_tiers for all to authenticated using (public.is_admin()) with check (public.is_admin());

-- Fumigation prices itself from here on, so it is no longer a manual quote and must not
-- have a property multiplier applied on top of a published figure.
update public.services
   set requires_review = false, uses_property_pricing = false, updated_at = now()
 where slug = 'fumigation';
...
select s.id, v.bedrooms, v.price
  ...
  join (values
    ...
    (7, 150000),
    (8, 170000)
  ) as v(bedrooms, price) on true
 where s.slug = 'fumigation'
...
select s.id, t.id, v.quantity, v.price
  from public.services s
  join public.space_types t on t.slug = 'boys-quarters'
  join (values
    (1, 30000::numeric),
    (2, 50000)
  ) as v(quantity, price) on true
 where s.slug = 'fumigation'
on conflict (service_id, space_type_id, quantity) do update set price = excluded.price, updated_at = now();

-- Nothing per-room may stack on top of the published fumigation table.
update public.service_space_prices sp
   set is_active = false, updated_at = now()
  from public.services s
 where sp.service_id = s.id and s.slug = 'fumigation' and sp.is_active;
...
declare
  ...
  space_tier_price numeric(12,2);
  space_has_tiers boolean;
begin
  ...
  for space_entry in select value from jsonb_array_elements(coalesce(request -> 'spaces', '[]'::jsonb)) loop
    ...
    -- A published quantity table beats a unit price: fumigation charges 30,000 for one BQ
    -- room and 50,000 for two, which no per-room figure reproduces.
    select exists (
      select 1 from public.service_space_tiers
       where service_id = service_row.id and space_type_id = space_row.id
    ) into space_has_tiers;

    if space_has_tiers then
      select price into space_tier_price
        from public.service_space_tiers
       where service_id = service_row.id and space_type_id = space_row.id and quantity = space_count;
      if found then
        running_subtotal := running_subtotal + space_tier_price;
        items := items || jsonb_build_object(
          'kind', 'SPACE',
          'label', format('%s × %s', space_row.name, space_count),
          'spaceTypeSlug', space_row.slug,
          'quantity', space_count, 'unitAmount', space_tier_price, 'amount', space_tier_price, 'sortOrder', item_order
        );
        item_order := item_order + 1;
      else
        -- Priced by table, but no row for this quantity: do not invent a figure.
        review_required := true;
        review_reasons := review_reasons || format('We quote %s in that quantity individually.', space_row.name);
      end if;
      continue;
    end if;
    ...
    if not found then
      -- A bedroom-tier price buys the whole home, so the living room, kitchen and bathrooms
      -- are already paid for. Treating them as "unpriced" sent every realistic deep-clean
      -- enquiry to manual review even though the tier had already answered the question.
      -- Only a service that prices room by room can have a genuinely unpriced room.
      if uses_tiers then
        continue;
      end if;
