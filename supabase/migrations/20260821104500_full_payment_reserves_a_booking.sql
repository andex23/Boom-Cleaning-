-- Reserving a booking takes the full amount, not a deposit.
--
-- The quote returned 30% of the total rounded up to the nearest NGN 500. BOOM reserves a
-- slot on full payment, so the figure the customer is asked for is simply the total.
--
-- Only the depositAmount expression changes. The rest of calculate_quote is carried over
-- verbatim from 014_fumigation_price_list.sql, because `create or replace function`
-- replaces the whole body and Postgres has no way to patch a single line of it.
