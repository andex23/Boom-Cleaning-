...
import { bookingPriceSchema, BookingPriceError, loadRecentBookingBreakdowns, setBookingPrice } from "@/features/pricing/pricing-admin";
...
export async function PATCH(request: Request) {
  ...
  if (!parsed.success) return NextResponse.json({ error: "Invalid price.", issues: z.flattenError(parsed.error) }, { status: 422, headers: noStore });

  try {
    const result = await setBookingPrice(parsed.data);
    return NextResponse.json({ result, bookings: await loadRecentBookingBreakdowns() }, { headers: noStore });
  } catch (error) {
    if (error instanceof BookingPriceError) return NextResponse.json({ error: error.message }, { status: 422, headers: noStore });
    console.error("Admin booking price update failed");
    return NextResponse.json({ error: "Unable to set the price." }, { status: 502, headers: noStore });
  }
}
